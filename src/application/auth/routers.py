...
@router.get("/email/verified", status_code=status.HTTP_200_OK, response_model=CheckEmailVerifiedResponse)
def check_email_verified(email: str, user_service: UserService = Depends(get_user_service)):
    is_verified = user_service.check_email_verified(email=email)
    return CheckEmailVerifiedResponse(is_verified=is_verified)
